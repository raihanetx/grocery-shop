      ...
      {categories.map((cat) => (
        <div key={cat.id} className="order-card category-row">
          <div className="order-grid-row">
            <div className="category-name-cell">
              <div style={{
                ...
                {cat.type === 'icon' ? (
                  ...
                  <img src={cat.image} style={{ width: '36px', height: '36px', objectFit: 'cover', borderRadius: '6px' }} alt={cat.name} />
                )}
              </div>
              <span>{cat.name}</span>
            </div>
            ...
            <div>{cat.created}</div>
            <div>
              <span className={`status-bracket ${cat.status === 'Active' ? 'active' : 'hidden-status'}`}>
                [{cat.status}]
              </span>
            </div>
